...
- 可以直接在 `## 窗影` 下像日记一样自然书写；想分段时按真实内容选写：`## 这一窗留给我的`、`## 我在想什么`、`## 关于小雨，关于我们`、`## 最近发生的事`、`## 还需要关心的事`。其他日记小标题会原样留在窗影正文里。
- 需要普通召回的经历放在 `## 想留下的记忆` 下，写成 `### scene | 作者标题 | cue：以后提到什么时召回`；可继续追加多个 `| cue：…`，没有就不写 Scene。
