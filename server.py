...
async def close_window(
    ...
    """写下一篇窗影和想留下的记忆。date 必须填写 `YYYY-MM-DD`；shadow 可直接写在 `## 窗影` 下，或选用 `这一窗留给我的`、`我在想什么`、`关于你，关于我们`、`最近发生的事`、`还需要关心的事`。Scene 写在 `## 想留下的记忆` 下；每一条的完整标记必须是 `### scene | 作者写的真实标题 | cue：一个召回入口 | cue：另一个召回入口`，标题不能省略，标题后的每一项都必须以 `cue：` 开头。合法示例：
...
Scene 正文用你的第一人称写成一个能独立理解的具体场景，保留实际发生的细节，也可以写下当时的情绪、欲望与犹豫，并保留引语原本人称，不要写成摘要或说明；每个 cue 是“以后提到什么时希望它回来”的入口，不是摘要。没有值得进入普通召回的经历时，可以只写窗影，不写 `## 想留下的记忆`。idempotency_key 用于重试；rejected_draft_* 按失败响应填写；continue_scene_index 选择续接 Scene。"""
    _ = context
