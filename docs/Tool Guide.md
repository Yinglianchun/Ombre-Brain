...
- content 用你的第一人称写成一个能独立理解的具体场景，保留实际发生的细节和引语原本人称，不要写成摘要或说明；不把正文里的“我”改成名字、AI、assistant 或第三人称；不加 `## Scene`、`### scene`、`### moment` 或固定模板。
...
- 可以直接在 `## 窗影` 下像日记一样写；想分段时可选：`这一窗留给我的`、`我在想什么`、`关于你，关于我们`、`最近发生的事`、`还需要关心的事`。其他小标题会留在窗影正文里。
- 需要普通召回的经历放进 `## 想留下的记忆`，写成 `### scene | 作者标题 | cue：一个召回入口 | cue：另一个召回入口`；Scene 正文继续用你的第一人称写成能独立理解的具体场景，保留实际发生的细节和引语原本人称，不写成摘要或说明；没有就不写 Scene。
